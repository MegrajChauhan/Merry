...
MerryOWC *merry_open_owc(MerryState *state) {
  ...
  /// NOTE: The _in_use flags refers to the state where at least one line is
  /// still open. NOTE: If both lines are closed, then the OWC is free. i.e
  /// _rclosed and _wclosed must be mtrue
  owc->_in_use = mtrue;
  owc->_rclosed = mfalse;
  owc->_wclosed = mfalse;
  owc->state.child_state = NULL;
  merry_assign_state(owc->state, _MERRY_ORIGIN_NONE_, 0);
  return owc;
}
...
mret_t merry_owc_reopen(MerryOWC *owc) {
  ...
  if (merry_open_pipe_owc(owc) == RET_FAILURE) {
    merry_assign_state(owc->state, _MERRY_INTERNAL_SYSTEM_ERROR_,
                       _MERRY_FAILED_TO_OPEN_PIPE_);
    return RET_FAILURE;
  }
...
mret_t merry_owc_speak(MerryOWC *owc, mbptr_t data, msize_t len) {
  ...
  if (write(owc->_write_line, (mptr_t)data, len) == -1) {
    merry_assign_state(owc->state, _MERRY_INTERNAL_SYSTEM_ERROR_,
                       _MERRY_REQUEST_TO_HOST_FAILED_);
    return RET_UNKNWON;
  }
  return RET_SUCCESS;
}
