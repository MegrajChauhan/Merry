...
MerryDynamicQueue *merry_dynamic_queue_init(MerryState *state) {
  ...
  merry_assign_state(queue->qstate, _MERRY_ORIGIN_NONE_, 0);
  queue->head = queue->tail = NULL;
  queue->data_count = 0;
  queue->qstate.child_state = NULL;
  return queue;
}
...
mret_t merry_dynamic_queue_push(MerryDynamicQueue *queue, mptr_t data) {
  ...
  if (!node) {
    merry_assign_state(queue->qstate, _MERRY_INTERNAL_SYSTEM_ERROR_,
                       _MERRY_MEM_ALLOCATION_FAILURE_);
    return RET_FAILURE;
  }
  node->data = data;
...
MerryStaticQueue *merry_static_queue_init(msize_t capacity, MerryState *state) {
  ...
  if (!queue) {
    merry_assign_state(*state, _MERRY_ORIGIN_NONE_,
                       _MERRY_MEM_ALLOCATION_FAILURE_);
    return RET_NULL;
  }
